...
const exportToExcel = async (nodes, edges) => {
  ...
  const icAoa = [
    ...
    [XS.c("Project", {bold:true,bg:"D9E1F2",fc:"1F3864",h:"left"}),
     XS.c("MBSE-COM — Engineering Management",{bg:"FFFFFF",h:"left"}),
     null,null,
...
// 타 PJT에서도 사용 가능하도록 설비 종류를 범용화
// 사양서 Import 시 Item 이름에서 Type을 유추, 애매하면 대표 범용기기로 분류
const EQUIPMENT_GENERIC = [
  // 핵심 Generic 6종
  { group:"Generic",   type:"Machine"      },  // 기본값 — 모든 미분류 설비
  { group:"Generic",   type:"Vessel"       },  // 용기/저장조 일반
  { group:"Generic",   type:"Pump"         },
  { group:"Generic",   type:"Valve"        },
  { group:"Generic",   type:"Line"         },  // 배관/라인 일반
  { group:"Generic",   type:"Structure"    },
  // 대표 범용기기 (이름 유추 매핑 대상)
  { group:"Equipment", type:"Tank"         },
  { group:"Equipment", type:"Vessel/Drum"  },
  { group:"Equipment", type:"Compressor"   },
  { group:"Equipment", type:"Fan/Blower"   },
  { group:"Equipment", type:"Heat Exchanger"},
  { group:"Equipment", type:"Filter"       },
  { group:"Equipment", type:"Reactor"      },
  { group:"Equipment", type:"Furnace"      },
  { group:"Equipment", type:"Duct"         },
  { group:"Equipment", type:"Conveyor"     },
  { group:"Equipment", type:"Package Unit" },
];
// 사이드바 그룹 구분용 (G=Generic, E=Equipment)
const EQUIPMENT_FBR = EQUIPMENT_GENERIC.map(e => ({ code: e.group==="Generic"?"G":"E", type: e.type }));
// 레거시 호환: 기존 MBSE-IC 모델을 COM에서 열어도 인식되도록 구 타입명 유지
const EQUIPMENT_LEGACY = [
  "Reactor","Bin/Vessel","Compressor/Fan","Cyclone/Scrubber","KO Drum","Feeder",
  "Valve/Gate","Cooling Tower","Burner/Nozzle","Bag Filter","Compensator","Mech. Filter",
  "Special Device","Hydraulic Unit","Refractory","Grid/Internals","Chute","Crane/Hoist",
  "Tower Struc.","Room Struc.","Pipe Rack","Stack","Piping","Foundation",
  "Decanter","Clarifier","Classifier","Feed Box","Chemical Dosing","Gas Duct",
  "Steel Structure","Scrubber","Feed Bin","Bucket Elev.","Bucket Elev","Pnumatic Conv.",
  "Stand Pipe","Bubbler","Riser","Hot Duct","Pond","Hopper","Pit","Cyclone","Drum Screen",
  "Structure/Stack","Conc. Struc.","Fan",
];
const EQUIPMENT_LIST = [...new Set([
  ...EQUIPMENT_GENERIC.map(e => e.type),
  ...EQUIPMENT_LEGACY,
])];

// ── MBSE-COM: 설비명에서 Block Type 유추 (애매하면 Machine) ──
// 사양서 Item 이름의 키워드로 대표 범용기기에 매핑
const TYPE_KEYWORDS = [
  // [정규식, Type] — 위에서부터 먼저 매칭 (구체적인 것 우선)
  [/\b(cooling\s*tower|쿨링타워|냉각탑)/i, "Heat Exchanger"],
  [/\b(pipe\s*rack|파이프\s*랙|rack)\b/i, "Structure"],
  [/\b(reactor|반응기|반응로)/i, "Reactor"],
  [/\b(furnace|kiln|smelter|소둔로|가열로|용해로|전기로)\b/i, "Furnace"],
  [/\b(heat\s*exchang|cooler|condenser|열교환|냉각기|응축)/i, "Heat Exchanger"],
  [/\b(compressor|압축)/i, "Compressor"],
  [/\b(fan|blower|송풍|배풍)/i, "Fan/Blower"],
  [/\b(pump|펌프)/i, "Pump"],
  [/\b(tank|저장조|저류조|수조)\b/i, "Tank"],
  [/\b(drum|vessel|용기|드럼|separator|knock|수액기)/i, "Vessel/Drum"],
  [/\b(filter|strainer|필터|여과|집진|bag\s*filter|scrubber|cyclone|demister)/i, "Filter"],
  [/\b(duct|덕트|연도|flue)\b/i, "Duct"],
  [/\b(conveyor|컨베이어|elevator|elev|feeder|피더|이송)/i, "Conveyor"],
  [/\b(valve|gate|밸브|댐퍼|damper)/i, "Valve"],
  [/\b(package|unit|skid|station|설비일식|module|hydraulic)/i, "Package Unit"],
  [/\b(structure|struc|건물|구조물|stack|tower|타워|building|foundation|기초|steel\s*str)/i, "Structure"],
  [/\b(pipe|piping|배관|line|라인|riser|standpipe|compensator|신축)/i, "Line"],
  [/\b(hopper|bin|silo|호퍼|사일로)/i, "Vessel/Drum"],
  [/\b(crane|hoist|크레인|호이스트)/i, "Machine"],
];
const guessTypeFromName = (name, itemNo) => {
  const hay = `${name||""} ${itemNo||""}`;
  for (const [re, type] of TYPE_KEYWORDS) {
    if (re.test(hay)) return type;
  }
  return "Machine"; // 애매하면 대표 범용기기
};
...
// ─────────────────────────────────────────────────────────────
const ICONS = {
  // ── MBSE-COM 범용 Generic 심볼 ──
  Machine:        <><rect x="4" y="6" width="16" height="13" rx="2" fill="none" stroke="currentColor" strokeWidth="1.5"/><circle cx="12" cy="12.5" r="3.2" fill="none" stroke="currentColor" strokeWidth="1.2"/><circle cx="12" cy="12.5" r="0.9" fill="currentColor"/><line x1="4" y1="19" x2="20" y2="19" stroke="currentColor" strokeWidth="1.8"/></>,
  Vessel:         <><rect x="7" y="4" width="10" height="16" rx="5" fill="none" stroke="currentColor" strokeWidth="1.5"/><line x1="7.5" y1="8" x2="16.5" y2="8" stroke="currentColor" strokeWidth="0.9"/><line x1="7.5" y1="16" x2="16.5" y2="16" stroke="currentColor" strokeWidth="0.9"/></>,
  Valve:          <><polygon points="3,6 12,12 3,18 Z" fill="none" stroke="currentColor" strokeWidth="1.5"/><polygon points="21,6 12,12 21,18 Z" fill="none" stroke="currentColor" strokeWidth="1.5"/><line x1="12" y1="12" x2="12" y2="5" stroke="currentColor" strokeWidth="1"/><line x1="9" y1="5" x2="15" y2="5" stroke="currentColor" strokeWidth="1.2"/></>,
  Line:           <><line x1="3" y1="12" x2="21" y2="12" stroke="currentColor" strokeWidth="2"/><circle cx="3" cy="12" r="1.6" fill="currentColor"/><circle cx="21" cy="12" r="1.6" fill="currentColor"/></>,
  Structure:      <><path d="M5,21 L5,5 L19,5 L19,21" fill="none" stroke="currentColor" strokeWidth="1.5"/><line x1="5" y1="5" x2="19" y2="13" stroke="currentColor" strokeWidth="0.9"/><line x1="19" y1="5" x2="5" y2="13" stroke="currentColor" strokeWidth="0.9"/><line x1="5" y1="13" x2="19" y2="13" stroke="currentColor" strokeWidth="0.9"/><line x1="3" y1="21" x2="21" y2="21" stroke="currentColor" strokeWidth="1.8"/></>,
  "Vessel/Drum":  <><rect x="3" y="7" width="18" height="10" rx="5" fill="none" stroke="currentColor" strokeWidth="1.5"/><line x1="9" y1="8" x2="9" y2="16" stroke="currentColor" strokeWidth="0.8"/><line x1="15" y1="8" x2="15" y2="16" stroke="currentColor" strokeWidth="0.8"/></>,
  "Fan/Blower":   <><circle cx="12" cy="12" r="8" fill="none" stroke="currentColor" strokeWidth="1.5"/><path d="M12,12 Q14,7 12,4.8 M12,12 Q17,14 19.2,12 M12,12 Q10,17 12,19.2 M12,12 Q7,10 4.8,12" fill="none" stroke="currentColor" strokeWidth="1.2"/><circle cx="12" cy="12" r="1.6" fill="currentColor"/></>,
  Furnace:        <><rect x="4" y="5" width="16" height="15" rx="2" fill="none" stroke="currentColor" strokeWidth="1.5"/><path d="M8,20 Q9,15 12,15 Q15,15 16,20" fill="none" stroke="currentColor" strokeWidth="1.1"/><path d="M10,12 Q11,9.5 10,7.5 M14,12 Q15,9.5 14,7.5" fill="none" stroke="currentColor" strokeWidth="1.1"/></>,
  "Package Unit": <><rect x="3" y="4" width="18" height="16" rx="2" fill="none" stroke="currentColor" strokeWidth="1.5" strokeDasharray="3 2"/><rect x="7" y="8" width="10" height="8" rx="1" fill="none" stroke="currentColor" strokeWidth="1.2"/><circle cx="12" cy="12" r="2" fill="none" stroke="currentColor" strokeWidth="1"/></>,
  Duct:           <><rect x="2.5" y="7" width="19" height="10" rx="1" fill="none" stroke="currentColor" strokeWidth="1.5"/><line x1="2.5" y1="10" x2="21.5" y2="10" stroke="currentColor" strokeWidth="0.8"/><line x1="2.5" y1="14" x2="21.5" y2="14" stroke="currentColor" strokeWidth="0.8"/><polygon points="19,12 16.8,10.7 16.8,13.3" fill="currentColor"/></>,
  ...
  // B: 저장조 — 상부 사각 + 하부 호퍼 콘
  "Bin/Vessel":   <><path d="M5,3 L19,3 L19,13 L14,21 L10,21 L5,13 Z" fill="none" stroke="currentColor" strokeWidth="1.5"/><line x1="5" y1="13" x2="19" y2="13" stroke="currentColor" strokeWidth="0.8"/></>,
  // C: 압축기/송풍기 — 원 + 회전 날개
  "Compressor/Fan":<><circle cx="12" cy="12" r="8" fill="none" stroke="currentColor" strokeWidth="1.5"/><line x1="12" y1="12" x2="12" y2="5" stroke="currentColor" strokeWidth="1.2"/><line x1="12" y1="12" x2="18" y2="15.5" stroke="currentColor" strokeWidth="1.2"/><line x1="12" y1="12" x2="6" y2="15.5" stroke="currentColor" strokeWidth="1.2"/></>,
  // D: 사이클론/스크러버 — 상부 원통 + 하부 역원뿔
  "Cyclone/Scrubber":<><path d="M6,3 L18,3 L18,10 L13,21 L11,21 L6,10 Z" fill="none" stroke="currentColor" strokeWidth="1.5"/><line x1="12" y1="3" x2="12" y2="7" stroke="currentColor" strokeWidth="1"/></>,
  // E: 녹아웃 드럼 — 가로 원통 + 데미스터 해칭
  "KO Drum":      <><rect x="3" y="7" width="18" height="10" rx="5" fill="none" stroke="currentColor" strokeWidth="1.5"/><line x1="9" y1="8" x2="9" y2="16" stroke="currentColor" strokeWidth="0.8" strokeDasharray="1.5 1"/><line x1="12" y1="8" x2="12" y2="16" stroke="currentColor" strokeWidth="0.8" strokeDasharray="1.5 1"/></>,
  // F: 피더 — 가로 사각 + 스크류 나선
  Feeder:         <><rect x="3" y="8" width="18" height="8" rx="1" fill="none" stroke="currentColor" strokeWidth="1.5"/><path d="M5,12 Q7,8.5 9,12 Q11,15.5 13,12 Q15,8.5 17,12 Q18,14 19,12" fill="none" stroke="currentColor" strokeWidth="1"/></>,
  // G: 밸브 — 보타이
  "Valve/Gate":   <><polygon points="3,6 12,12 3,18 Z" fill="none" stroke="currentColor" strokeWidth="1.5"/><polygon points="21,6 12,12 21,18 Z" fill="none" stroke="currentColor" strokeWidth="1.5"/><line x1="12" y1="12" x2="12" y2="5" stroke="currentColor" strokeWidth="1"/><line x1="9" y1="5" x2="15" y2="5" stroke="currentColor" strokeWidth="1.2"/></>,
  // J: 버너 — 노즐 + 화염
  "Burner/Nozzle":<><path d="M3,9 L11,9 L15,12 L11,15 L3,15 Z" fill="none" stroke="currentColor" strokeWidth="1.5"/><path d="M15,12 Q17,9 19,12 Q21,14.5 18.5,16 Q19.5,13.5 17,13.5 Q16,14.5 15,12" fill="none" stroke="currentColor" strokeWidth="1.2"/></>,
  // K: 백필터 — 사각 + 세로 백 + 상부 출구
  "Bag Filter":   <><rect x="4" y="6" width="16" height="15" rx="1" fill="none" stroke="currentColor" strokeWidth="1.5"/><line x1="8" y1="8" x2="8" y2="18" stroke="currentColor" strokeWidth="1"/><line x1="12" y1="8" x2="12" y2="18" stroke="currentColor" strokeWidth="1"/><line x1="16" y1="8" x2="16" y2="18" stroke="currentColor" strokeWidth="1"/><line x1="12" y1="6" x2="12" y2="3" stroke="currentColor" strokeWidth="1.2"/></>,
  // L: 신축이음 — 배관 + 벨로우즈 주름
  Compensator:    <><line x1="2" y1="12" x2="7" y2="12" stroke="currentColor" strokeWidth="1.5"/><path d="M7,12 Q8.5,6 10,12 Q11.5,18 13,12 Q14.5,6 16,12" fill="none" stroke="currentColor" strokeWidth="1.3"/><line x1="16" y1="12" x2="22" y2="12" stroke="currentColor" strokeWidth="1.5"/></>,
  // M: 기계식 필터 — 원 + 내부 필터 V
  "Mech. Filter": <><circle cx="12" cy="12" r="8" fill="none" stroke="currentColor" strokeWidth="1.5"/><polyline points="7,9 12,15 17,9" fill="none" stroke="currentColor" strokeWidth="1.2"/></>,
  // N: 특수설비 — 사각 + 별 표시 (Vibrator/Sampler/Flare/Decanter)
  "Special Device":<><rect x="4" y="4" width="16" height="16" rx="2" fill="none" stroke="currentColor" strokeWidth="1.5"/><line x1="12" y1="7" x2="12" y2="17" stroke="currentColor" strokeWidth="1.1"/><line x1="7.7" y1="9.5" x2="16.3" y2="14.5" stroke="currentColor" strokeWidth="1.1"/><line x1="16.3" y1="9.5" x2="7.7" y2="14.5" stroke="currentColor" strokeWidth="1.1"/></>,
  // Q: 유압유닛 — 탱크 + 게이지
  "Hydraulic Unit":<><rect x="4" y="9" width="16" height="11" rx="1.5" fill="none" stroke="currentColor" strokeWidth="1.5"/><circle cx="12" cy="6" r="2.6" fill="none" stroke="currentColor" strokeWidth="1.2"/><line x1="12" y1="6" x2="13.6" y2="4.6" stroke="currentColor" strokeWidth="1"/><path d="M6,14 Q9,12.5 12,14 Q15,15.5 18,14" fill="none" stroke="currentColor" strokeWidth="1"/></>,
  // R: 내화물 — 벽돌 패턴
  Refractory:     <><rect x="3" y="5" width="18" height="14" fill="none" stroke="currentColor" strokeWidth="1.5"/><line x1="3" y1="9.7" x2="21" y2="9.7" stroke="currentColor" strokeWidth="0.9"/><line x1="3" y1="14.3" x2="21" y2="14.3" stroke="currentColor" strokeWidth="0.9"/><line x1="10" y1="5" x2="10" y2="9.7" stroke="currentColor" strokeWidth="0.9"/><line x1="15" y1="9.7" x2="15" y2="14.3" stroke="currentColor" strokeWidth="0.9"/><line x1="9" y1="14.3" x2="9" y2="19" stroke="currentColor" strokeWidth="0.9"/></>,
  // T: 그리드/내부장치 — 격자
  "Grid/Internals":<><rect x="4" y="4" width="16" height="16" rx="1" fill="none" stroke="currentColor" strokeWidth="1.5"/><line x1="9.3" y1="4" x2="9.3" y2="20" stroke="currentColor" strokeWidth="0.9"/><line x1="14.7" y1="4" x2="14.7" y2="20" stroke="currentColor" strokeWidth="0.9"/><line x1="4" y1="9.3" x2="20" y2="9.3" stroke="currentColor" strokeWidth="0.9"/><line x1="4" y1="14.7" x2="20" y2="14.7" stroke="currentColor" strokeWidth="0.9"/></>,
  // U: 슈트 — 경사 평행사변형 + 흐름 화살표
  Chute:          <><polygon points="4,4 11,4 20,17 13,17" fill="none" stroke="currentColor" strokeWidth="1.5"/><line x1="14" y1="19" x2="18" y2="21" stroke="currentColor" strokeWidth="1.2"/><polygon points="18,21 16.2,20.9 17.2,19.4" fill="currentColor"/></>,
  // X: 크레인/호이스트 — 빔 + 와이어 + 후크
  "Crane/Hoist":  <><line x1="3" y1="4" x2="21" y2="4" stroke="currentColor" strokeWidth="1.8"/><line x1="12" y1="4" x2="12" y2="13" stroke="currentColor" strokeWidth="1.2"/><path d="M12,13 Q12,17 9.5,17 Q7.5,17 8,15" fill="none" stroke="currentColor" strokeWidth="1.4"/></>,
  // Y: 구조물/스택 — 타워 + 굴뚝
...
const Sidebar = memo(({ onDragStart }) => {
  ...
  return (
    <div style={{ width:195,background:"#f8fafc",borderRight:"1px solid #e2e8f0",overflowY:"auto",flexShrink:0 }}>
      ...
      <Sec title="Equipment (Block Library)" cat="EquipFBR">
        {EQUIPMENT_FBR.map(({code, type}, idx)=>{
          const prevCode = idx>0 ? EQUIPMENT_FBR[idx-1].code : null;
          const isGen = code==="G";
          return (
            <React.Fragment key={`${code}_${type}`}>
              {/* 그룹 구분선 (Generic ↔ Equipment) */}
              {prevCode !== null && prevCode !== code && (
                <div style={{ height:1, background:"#e2e8f0", margin:"4px 0 2px" }}/>
              )}
              <div draggable onDragStart={e=>onDragStart(e,"equipment",type)}
                style={iS(isGen?"#0891b2":"#b45309")} {...hov}
                title={isGen?"Generic — 범용 기본 Block":"대표 범용기기"}>
                <span style={{
                  display:"inline-flex", alignItems:"center", justifyContent:"center",
                  minWidth:26, height:15, borderRadius:3, flexShrink:0, padding:"0 3px",
                  background: isGen?"#cffafe":"#fef3c7",
                  color: isGen?"#0e7490":"#92400e",
                  fontSize:8, fontWeight:800, fontFamily:"sans-serif",
                  border:`1px solid ${isGen?"#67e8f9":"#fcd34d"}`,
                }}>{isGen?"GEN":"EQUIP"}</span>
...
const CanvasInner = () => {
  ...
  const [currentUser, setCurrentUser] = useState(
    () => localStorage.getItem("mbsecom_current_user") || ""
  );
  ...
  useEffect(()=>{
    try {
      const saved = localStorage.getItem("mbsecom_autosave");
      if(saved){
  ...
  useEffect(()=>{
    ...
    autoSaveTimer.current = setTimeout(()=>{
      try {
        localStorage.setItem("mbsecom_autosave", JSON.stringify({ nodes, edges, history, projectMeta }));
  ...
  const confirmSave = (author, summary) => {
    ...
    setHistory(h=>{
      ...
      localStorage.setItem("mbsecom_autosave",JSON.stringify({ nodes,edges,history:updated,projectMeta }));
      return updated;
    });
  ...
  // ── 사양서 적용 (최초 생성 / 수정 반영 분기) ──────
  const applySpecImport = useCallback((parsedBlocks) => {
    if (!parsedBlocks || parsedBlocks.length === 0) { setShowSpecImport(false); return; }

    // 설비명에서 Block 종류 유추 (Type 컬럼 우선 → 이름 키워드 → Machine)
    const guessType = (b) => {
      // 사양서에 Type/종류 컬럼이 있으면 우선 사용
      const explicit = b.equipType || b.type || b.Type || b["종류"] || b["설비종류"];
      if (explicit && EQUIPMENT_LIST.includes(String(explicit).trim()))
    ...
    if (projectMeta.revision === 0 && !anyMatch) {
      ...
      parsedBlocks.forEach(b => {
        ...
        let g;
        if (hasNumericPrefix) {
          g = (String(itemNo).match(/^([A-Za-z0-9]{1,3})/) || [,"기타"])[1];
        } else {
          g = guessType(b); // Type별 구획
        }
  ...
  //  해법: ① 작성자가 "📤 배포 JSON"으로 mbsecom-published.json 다운로드
  ...
  const onPublishExport = () => {
    ...
    const a = document.createElement("a"); a.href = URL.createObjectURL(blob);
    a.download = "mbsecom-published.json"; a.click();
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await fetch(`mbsecom-published.json?t=${Date.now()}`, { cache:"no-store" });
        if (!res.ok) return; // 파일 미배포 — 조용히 무시
        const data = await res.json();
        if (!data?.publishedAt || !Array.isArray(data.nodes)) return;
        const applied = localStorage.getItem("mbsecom_published_applied");
  ...
  const applyPublished = () => {
    ...
    localStorage.setItem("mbsecom_published_applied", publishedUpdate.publishedAt);
    ...
    <div style={{ display:"flex",height:"100vh",width:"100vw",fontFamily:"'Segoe UI',sans-serif",background:"#f8fafc",overflow:"hidden" }}>
      ...
      <div style={{ flex:1,display:"flex",flexDirection:"column",minWidth:0 }}>
        <div style={{ height:44,background:"#0f172a",display:"flex",alignItems:"center",padding:"0 16px",gap:8,flexShrink:0 }}>
          <span style={{ color:"#f1f5f9",fontWeight:800,fontSize:14,marginRight:6,letterSpacing:0.5 }}>⬡ MBSE-COM</span>
          ...
          <button onClick={onPublishExport}
            ...
            title="현재 모델을 mbsecom-published.json으로 다운로드 → GitHub public/ 폴더에 커밋하면 모든 Teams 참가자에게 업데이트 알림이 표시됩니다">
            📤 배포
          </button>
                ...
                <div style={{ display:"flex",gap:6 }}>
                  <button onClick={()=>{
                    if(window.confirm("저장 이력을 전체 삭제할까요?")) {
                      setHistory([]);
                      localStorage.removeItem("mbsecom_autosave");
                    }
            ...
            <div style={{ padding:"10px 12px", display:"flex", flexDirection:"column", gap:8, maxHeight:"60vh", overflowY:"auto" }}>
              ...
              <div>
                ...
                <input value={currentUser} placeholder="이름 입력"
                  onChange={e=>{ setCurrentUser(e.target.value); localStorage.setItem("mbsecom_current_user", e.target.value); }}
